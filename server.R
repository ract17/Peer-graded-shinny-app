...
shinyServer(function(input, output) {
    ...
    output$distPlot <- renderPlot({
        ...
        plot<-ggplot(data=diam, aes(x=carat, y=price))+geom_point(aes(color=cut),alpa=0.3)+
          geom_smooth(method="lm")+ 
          geom_vline(xintercept = input$car, color="red")+geom_hline(yintercept=pred,color="green")
    ...
    output$result <-renderText({
      #Renders the text for the prediction bleow the graph
